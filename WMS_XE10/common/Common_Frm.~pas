...
uses SysUtils, Forms, Dialogs, Windows, ExtCtrls, Grids, FileUtil, Buttons,
     Classes, StdCtrls, Messages, Graphics, Controls, QMask, Mask, MaskUtils,
     ComCtrls, CurrEdit,Imm;
type
...
begin
    If iKey = VK_PRIOR Then result := Control.Perform(EM_SCROLL,SB_PAGEUP,0);
    If iKey = VK_NEXT  Then result := Control.Perform(EM_SCROLL,SB_PAGEDOWN,0);
end;
...
//        iCnt : Integer;
    Begin
//        iCnt :=0;
//        If  Not(bSts) Then iCnt := giMaxCnt+2;
...
begin
    ...
    With (gChForm) Do Begin
       If assigned(gOldEdit)       Then Begin
          With  gOldEdit     As TEdit       Do  Color      := rChColor;
       End;
