...
procedure TFIStore_Back.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFIStore_Back.Button4Click(Sender: TObject);
begin
  Close;
end;

procedure TFIStore_Back.Button1Click(Sender: TObject);
begin
  pr_qryOpen('%%');
end;

procedure TFIStore_Back.Button2Click(Sender: TObject);
begin
  DBGrid2Excel('', DBGrid1);
end;

procedure TFIStore_Back.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFIStore_Back.FormDestroy(Sender: TObject);
begin
  FIStore_Back := nil;
end;

procedure TFIStore_Back.FormCreate(Sender: TObject);
begin
  edBarcode.Clear;
  ...
  giCount := 0;
  //gsJOBS  := gcStore_Back;
  gsJOBS  := AITEM_STATE[gcStore_Back];
  Button1Click(Self);
end;

procedure TFIStore_Back.Button3Click(Sender: TObject);
var
...
begin                        
  ...
  with DM.qryOpen do try
    ...
    SQL.Add(AColum_Name[gcStore_Back]);
    SQL.Add(' = :prdt_date                       '+
...
procedure TFIStore_Back.Button5Click(Sender: TObject);
var
  lsORGAN_CODE : String;
begin
{
...
procedure TFIStore_Back.edBarcodeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFIStore_Back.pr_qryOpen(ABarcode:String);
begin
  qryMain.Close;
