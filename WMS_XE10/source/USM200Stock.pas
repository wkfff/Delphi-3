...
uses
  ...
  AdvGrid, DBTables, ComCtrls, Buttons, AdvObj;

type
  TFSM200Stock = class(TForm)
    grpS01: TGroupBox;
    Label3: TLabel;
    Label14: TLabel;
    Label4: TLabel;
...
procedure TFSM200Stock.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  DM.qrMT100.Close;
...
procedure TFSM200Stock.Button4Click(Sender: TObject);
begin
  Close;
end;

procedure TFSM200Stock.FormCreate(Sender: TObject);
begin
...
procedure TFSM200Stock.Button2Click(Sender: TObject);
begin
  fn_Save2Excell(Self, sgMain);
end;

procedure TFSM200Stock.edCodeKeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then SelectNext(sender as tWincontrol, true, true);
  if key = #13 then key := #0;
end;

procedure TFSM200Stock.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFSM200Stock.FormDestroy(Sender: TObject);
begin
  FSM200Stock := nil;
end;

procedure TFSM200Stock.pr_Clear;
begin

end;

procedure TFSM200Stock.pr_Grid_Init;
begin
...
procedure TFSM200Stock.pr_qryOpen;
var
  la : Array of Integer;
begin
  pr_ClearGrid(sgMain);
...
procedure TFSM200Stock.pr_Ready;
begin

end;

procedure TFSM200Stock.sgMainGetAlignment(Sender: TObject; ARow, ACol: Integer;
  var HAlign: TAlignment; var VAlign: TVAlignment);
begin
    HAlign := taCenter;
...
procedure TFSM200Stock.sgMainGetCellColor(Sender: TObject; ARow, ACol: Integer;
  AState: TGridDrawState; ABrush: TBrush; AFont: TFont);
begin
      {
...
procedure TFSM200Stock.Button1Click(Sender: TObject);
begin
...
procedure TFSM200Stock.sbnLine_S01Click(Sender: TObject);
begin
  try
...
procedure TFSM200Stock.FormActivate(Sender: TObject);
begin
  FQMain.fn_ChildShow(TFSM200Stock ,'FSM200Stock');
end;

procedure TFSM200Stock.rbSt_AllClick(Sender: TObject);
begin
  Button1.Click;
end;
