...
type
  TFIAs_Back = class(TForm)
    ...
    Label8: TLabel;
    DBMemo1: TDBMemo;
    qryMainREMARKS_AS: TStringField;
    Panel1: TPanel;
    pnlMessage: TPanel;
    Panel3: TPanel;
    Panel4: TPanel;
...
procedure TFIAs_Back.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFIAs_Back.Button4Click(Sender: TObject);
begin
  Close;
end;

procedure TFIAs_Back.Button1Click(Sender: TObject);
begin
  pr_qryOpen('%%');
end;

procedure TFIAs_Back.Button2Click(Sender: TObject);
begin
  DBGrid2Excel('', DBGrid1);
end;

procedure TFIAs_Back.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFIAs_Back.FormDestroy(Sender: TObject);
begin
  FIAs_Back := nil;
end;

procedure TFIAs_Back.FormCreate(Sender: TObject);
begin
  edBarcode.Clear;
  ...
  giCount := 0;
  //gsJOBS  := gcAs_Back;
  gsJOBS  := AITEM_STATE[gcAs_Back];
  Button1Click(Self);
end;

procedure TFIAs_Back.Button3Click(Sender: TObject);
var
...
begin                           
  ...
  with DM.qryOpen do try
    ...
    if Edit1.Text = '' then begin
      pr_SetMessage(Panel3, Label7.Caption+'가 없습니다.');
      Edit1.SetFocus;
      Exit;
    end;
    ...
    SQL.Add(AColum_Name[gcAs_Back]);
    SQL.Add(' = :prdt_date                       '+
            '    where barcode = :barcode                                            '+
            '                                                                        '+
            '   insert into spd101                                                   '+
            '   ([STDT_DATE],[BARCODE],[ORGAN_CODE],[ITEM_CODE],[ITEM_STATE],[I_ID], REMARKS) '+
            '   values                                                               '+
            '   (:prdt_date, :barcode, :ORGAN_CODE, :ITEM_CODE, :ITEM_STATE, :I_ID, :REMARKS ) ');
...
procedure TFIAs_Back.Button5Click(Sender: TObject);
var
  lsORGAN_CODE : String;
begin
{
...
procedure TFIAs_Back.edBarcodeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFIAs_Back.pr_qryOpen(ABarcode:String);
begin
  qryMain.Close;
